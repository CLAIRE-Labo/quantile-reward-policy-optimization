...
# SSH to a cluster.
ssh clariden
mkdir -p $HOME/projects/qrpo
cd $HOME/projects/qrpo
git clone git@github.com:CLAIRE-Labo/quantile-reward-policy-optimization.git dev
git clone git@github.com:CLAIRE-Labo/quantile-reward-policy-optimization.git run

# To setup symlinks to the scratch storage you can run the following commands
mkdir -p $SCRATCH/projects/qrpo/data/dev
mkdir -p $SCRATCH/projects/qrpo/outputs/dev

for instance in run dev; do
  ln -s $SCRATCH/projects/qrpo/data/dev $HOME/projects/qrpo/$instance/data/dev
  ln -s $SCRATCH/projects/qrpo/outputs/dev $HOME/projects/qrpo/$instance/outputs/dev
  # We also have a shared directory maintained on smoalla's scratch
  ln -s /iopsstor/scratch/cscs/smoalla/projects/qrpo/data/shared $HOME/projects/qrpo/$instance/data/shared
  ln -s /iopsstor/scratch/cscs/smoalla/projects/qrpo/outputs/shared $HOME/projects/qrpo/$instance/outputs/shared
done
```
...
You will find the image to use for this project at `/capstor/store/cscs/swissai/a10/container-images/a10+smoalla+qrpo+arm64-cuda-root-latest.sqsh`.
...
CONTAINER_IMAGES=$SCRATCH/container-images
mkdir -p $CONTAINER_IMAGES
cp /capstor/store/cscs/swissai/a10/container-images/a10+smoalla+qrpo+arm64-cuda-root-latest.sqsh $CONTAINER_IMAGES/a10+$(id -un)+qrpo+arm64-cuda-root-latest.sqsh
```
