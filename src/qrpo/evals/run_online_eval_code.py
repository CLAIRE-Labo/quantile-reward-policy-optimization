...
def compute_rewards_for_rows(reward_model, tokenizer, batch, config):
    ...
    batch_ref_model_completions = [
        [sample[1]["content"]] for sample in batch["prompt_and_completion"]
    ]  # [B, 1]
    batch_tests = batch["test"]  # [B]

    # Extract the tokenized sequences without padding
    all_logps_ref_completions_reward_tokens = []
    all_ref_completions_reward_tokens_len = []
    all_ref_completions_reward_texts = []

    tokenized_inputs = tokenizer.apply_chat_template(
        batch["prompt_and_completion"],
        return_tensors="pt",
        padding=True,
        return_dict=True,
        truncation=True,
    ...
    for j in range(len(tokenized_inputs["attention_mask"])):
        ...
        all_logps_ref_completions_reward_tokens.append(tokens)
        all_ref_completions_reward_tokens_len.append(tokens_len)
        all_ref_completions_reward_texts.append(
            tokenizer.decode(tokens, skip_special_tokens=False)
        )

    batch_ref_rewards = reward_model.execute_batch_code(
        pre_code,
        batch_ref_model_completions,  # [B, Nref]
        batch_entrypoints,  # [B]
        batch_tests,  # [B]
        config.reward_model_args,
    )  # [B, Nref] = [B, 1]
    all_ref_rewards = []
    for i in range(len(batch_ref_rewards)):
        all_ref_rewards.append(batch_ref_rewards[i][0])
    return (
        all_ref_rewards,
        all_logps_ref_completions_reward_tokens,
        all_ref_completions_reward_tokens_len,
...
def compute_rewards_batch(reward_model, tokenizer, data, config):
    ...
    for i in trange(0, len(data), config.compute_reward_config.batch_size):
        batch = data.select(
            range(i, min(i + config.compute_reward_config.batch_size, len(data)))
        )
        (
            batch_rewards,
...
def compute_rewards(data, config):
    """
    Function to compute rewards for completions.
    ...
    return compute_rewards_batch(reward_model, tokenizer, data, config)
...
@hydra.main(
    version_base=None,
    config_path="../configs",
    config_name="run-online-eval-code",
)
    ...
    for seed in seeds:
        ...
        for checkpoint in checkpoints:
            ...
            (
                ...
                all_reward_texts,
            ) = compute_rewards(completions_dataset, config)
